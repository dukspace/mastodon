# Verdant Canopy theme

Verdant Canopy is an optional color theme for Mastodon v4.6.6. It keeps the
default Mastodon layout, typography, and component styles while replacing the
neutral and brand color palettes with shaded foliage, pale sage, and sunlit
leaf tones.
...
Verdant Canopy imports the default `styles/application.scss` entrypoint and
then overrides only CSS custom properties. Mastodon's error, warning, and
success colors are left unchanged so that semantic states keep their familiar
meaning and relative hierarchy.

The dark scheme adds subtle separation between adjacent forest-toned surfaces.
High contrast keeps container and control borders strong while using quieter
row dividers. The light scheme uses pale sage surfaces and a subdued custom
scrollbar instead of pure white and a visually dominant thumb.
...
`data-color-scheme` and `data-contrast` HTML attributes, so the same Verdant
Canopy theme supports Auto, Light, Dark, and High contrast preferences.
...
`verdant_canopy.scss` are updated by the Vite development server without an
asset precompile.
...
The core palette combinations have these contrast ratios:

- Dark primary text: 15.27:1
- Dark secondary text: 9.58:1
- Dark brand text: 10.85:1
- Light primary text: 16.83:1
- Light secondary text: 5.96:1
- Light brand text: 6.28:1
- White text on the primary brand button: 6.74:1
